    ...
    while ( !openSet.empty() ){
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){
           ...
           double gn = currentPtr->gScore + edgeCostSets[i];
           double fn = gn + getHeu(neighborPtr, endPtr, startPtr, endPtr);
            if(neighborPtr -> id != 1 && neighborPtr->id != -1){ //discover a new node
                /*
                *
                *
